...
module mac_ctrl
       (
         ...
         output reg [15:0]         udp_send_data_length,//UDP发送数据长度
         input      [31:0]         ip_rec_source_ip_addr,    //broadcast source ip address
         output reg [31:0]         destination_ip_addr,//目的IP地址
                                   
         input      [15:0]         fifo_data,             //从FIFO中读出ADC数据
         input      [10:0]         fifo_data_count,       //FIFO可读数据量
         output                    fifo_rd_en,            //FIFO读使能?
         input                     udp_rd_en,//udp读请求
                                   
         input      [7:0]          header,//命令头
         input      [31:0]         sample_num,//采样长度
         output reg [31:0]         sample_len,//输出采样长度
                                   
         input      [7:0]          reply_data,         //reply cmd
         output reg                cmd_reply_ack,//命令应答响应
         input                     cmd_reply_req,      //cmd reply request
         input      [15:0]         cmd_send_len,       //cmd reply length
         output reg                ad_data_ack,//ADC数据应答信号
         input                     ad_data_req,        //ad data request
         output reg                ad_sample_req,//ADC采样请求
         input                     ad_sample_ack,//ADC采样应答
                                   
         input                     mac_send_end,//mac发送结信号
         input                     mac_not_exist,//IP对应MAC不存在
         input                     arp_found,//接收到MAC
         output                    udp_tx_req,//UDP发送请求
         output                    arp_request_req,//ARP发送请求
         output     [7:0]          udp_data,//UDP发送数据
                                   
         input                     read_req_ack,//读FIFO应答
         output reg                read_req,//读FIFO请求
         output reg [15:0]         identify_code//ＩＰ序列号
